...
connect4_sampled_efficientzero_config = dict(
    exp_name=f'data_sez/connect4_sampled_efficientzero_bot_seed0',
    env=dict(
    ...
    policy=dict(
        ...
        # Bot mode uses to_play=-1, so this path is less adversarial than self-play.
        # Python tree is still the safer first choice.
        mcts_ctree=mcts_ctree,

        game_segment_length=int(6 * 7 / 2),
        ...
        # In bot mode, one agent step includes agent move + bot response,
        # so the effective agent horizon is about half the full board.
        td_steps=int(6 * 7 / 2),
        discount_factor=1,
